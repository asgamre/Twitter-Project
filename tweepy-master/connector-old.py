...
from mysqlconnector import errorcode
import string
import time
import json
import nltk
...
from nltkbernoulli import *
import re
from nltktrainedobj import *
from stringmatching import *
import pickle
# replace mysqlserver with "localhost" 
#  you are running via your own server
# server       MySQL username	MySQL pass  Database name

# cnx = mysqlconnectorconnect(user='root', password='rambo',
                              # host='localhost',
                              # database='Test',
                              # charset = 'utf8mb4')
# cursor=cnxcursor()
...
nltkdataload('tokenizers/punkt/englishpickle')
stop_words = set(stopwordswords("english"))
filtered_tweet = []
hashtags = []
...
class listener(StreamListener):
	...
	def __init__(self):
		...
		if(trainingRequired):
			(ber_obj,classifier) = trainfunc()
			outfile = open('classifierDumpFilep', 'wb') 
			pickledump((ber_obj,classifier) , outfile) 
			outfileclose()
			print "TrainingRequiredTrue"
		else: 
			f1 = open('classifierDumpFilep') 
			if(f1): 
				(ber_obj,classifier) = pickleload(f1) 
				f1close() 
				print ("Loaded Trained Data")
		#print classifierprior
		#classifier1=TrainedObject(classifierword_list,classifierprior,classifiercondprob)
		#print ber_objword_list
		#print isinstance(selfber_obj,BernoulliNBClassifier)
		#print isinstance(classifier1,TrainedObject)
	...
	def on_data(self, data):
		...
		if 'text' in all_data:
			  ...
			  #  rematch(r'#?([Aa]ccident)',tweet) is not None
			  ...
			  tweet = tweetreplace("#","")	
			  result = resub(r"http\S+", "", tweet)
			  words = word_tokenize(result)
			  for w in words:
				   if w not in stop_words:
					  filtered_tweetappend(w)
			  tweetstr = ' '.join(filtered_tweet)
			  if 'accident' in tweet:
				tweetarray=tweetsplit(" ")
				for  in tweetarray
					tweetarrayappend(spellchecker())
				classfunc(tweetarray,selfber_obj,selfclassifier)
				print tweetarray
				print "Tweet classified..."
				# print(tweetstr,hashtags)
				
				test_dataset =  [('major accident bkc link road casualties reported yet sad', 'situational'),
								 ...
								 ('woman killed  weekend car crash bandra caraccident majoraccident vehiclecrash', 'situational'),
								 ('bandra main road congested due  car accident hopeless traffic', 'situational'),
								 ('bandra railway accident ', 'situational'),
								 ('bandra western express highway par huwa accident ', 'situational'),
								 ('train accident  bandra railway station 2dae eve  human amputated into half  mumbai safe travel', 'situational'),
								 ('trafflinemum harbour line from bandra running letsomething might happenaccident', 'situational'),
								 ('insane accident  bandra east  mumbai bus rams into shops damaging  badly  one hurt crash news', 'situational'),
								 ('best bus overturned  bandra flyover accident traffic mumbai ', 'situational'),
								 ...
								 ('fire  borivali then  accident  the weh just  average saturday night', 'situational'),
								 ...
								 ('roadaccident accident mumbai hitandrun hitandrunaccident borivali', 'situational'),
								 ...
								 ('fire accidents  jvlr result  major loss  property and goods  casualties reported', 'situational'),
								 ('fire personal working  get the driver out accident jvlr smart_mumbaikar trafflinemum', 'situational'),
								 ('accident  jvlr ', 'situational'),
								 ('accident near plaza jvlr mumbai caused lot  traffic jam idiots  plaza cinema jvlr ', 'situational'),
								 ...
								 ('dadar   its worst again accident traffic late', 'situational'),
								 ('trafflinemum dadar flyover weh  slow moving accident opp nesco', 'situational'),
								 ...
								 ('stuck jus weh station for last mins mumbailocal accident', 'situational'),
								 ('pretty bad accident  weh  south bound traffic  western express highway  hope  one critical', 'situational'),
								 ('accident  jogeshwari ', 'situational'),
								 ('mad traffic jogeshwari mumbai traffic accident morning mumairport ', 'situational'),
								 ('trafflinemum flyover jogeshwari  / b slow moving accident opp nesco', 'situational'),
								 ...
								 ('this just  vashis bridge  doom multiple accidents since the last two months mumbai ', 'situational'),
								 ...
								 ('ronshah withdraws equivalent dakar after stupid accident', 'non-situational'),
								 ...
								 ('the roncar had its first accident ', 'non-situational'),
								 ...
								 ('shocking ron narrowly escapes  car accident', 'non-situational'),
								 ...
								 ('road safety how your smartness can help   auto accident ', 'non-situational'),
								 ...
								 ('whoah many police ambulance fire  banstead crossroads banstead accident', 'non-situational'),
								 ('these inventions werent meant  happen via list25 accident', 'non-situational'),
								 ('crash accident scoop incredible  photos  this accident made  news today', 'non-situational'),
								 ...
								 ('smriti irani gets herself  just  right position when she lies accident ', 'non-situational'),
								 ...
								 ('spotted accident thanks god everyone  safe minor rain slipperyroad', 'non-situational'),
								 ...
								 ('when people get   accident scene  first thing they   check what they can steal', 'non-situational'),
								 ...
								 ('how you can prevent form the accident with heavyvehicles allahrehamkare ', 'non-situational'),
				...
				print "accuracy:"
				print ber_objaccuracy(list_class,classifier,test_dataset)
...
authset_access_token(atoken, asecret)

twitterStream = Stream(auth, listener())
twitterStreamfilter(locations=[72,18,73,20],languages = ["en"], stall_warnings = True)